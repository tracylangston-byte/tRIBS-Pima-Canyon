...
plot_lhs_11param.py
===================
Generates all diagnostic figures from the 11-parameter LHS sweep (series 83).
Adds channelwidthcoeff and thetaS_mult as new swept dimensions on top of the
6-param series 82 layout, and replaces the psiB deep-dive with a 3-panel
new-parameter deep-dive.

Usage (run from the smf_demo directory):
    python plot_lhs_11param.py

Produces 11 figures saved to:
...
-----------
    ...
    fig3_correlation_bar.png            — Pearson r of each swept param vs KGE (7 bars)
    fig4_parallel_coordinates.png       — All 7 params as vertical axes, lines colored by KGE
    fig5_pairwise_scatter.png           — 21-panel pairwise scatter matrix, colored by KGE
    fig6_kge_vs_each_param.png          — 7-panel KGE vs each parameter (1D response curves)
    ...
    fig10_new_params_deepdive.png       — 3-panel deep-dive on channelwidthcoeff and thetaS_mult:
                                           (a) KGE vs channelwidthcoeff, colored by Ks
                                           (b) KGE vs thetaS_mult, colored by Ks
                                           (c) channelwidthcoeff × thetaS_mult, colored by KGE
    fig11_psib_sensitivity.png          — 3-panel psiB_mult deep-dive (carried from series 82):
                                           (a) KGE vs psiB_mult, colored by Ks
                                           (b) psiB_mult × Ks_mult, colored by KGE
...
if not results_path.exists():
    raise FileNotFoundError(
        ...
        f"Run run_lhs_11param.py first."
    )
...
required_cols = [
    "run_id", "Ks_mult", "kinemvelcoef", "flowexp", "channelroughness",
...
if missing:
    raise ValueError(
        f"Missing columns in results CSV: {missing}\n"
...
SERIES_TITLE = f"Series 83  |  Ks {ks_lo:.1f}–{ks_hi:.1f}x  |  7 free params"
...
cw_vals    = df["channelwidthcoeff"].values
ths_vals   = df["thetaS_mult"].values
...
# Swept parameter metadata — 7 params in display order
PARAM_META = {
    ...
    "channelwidthcoeff": {"label": "Channel width coeff. \u03b1B",     "vals": cw_vals,   "fmt": "{:.3f}"},
    "thetaS_mult":       {"label": "Saturated moisture \u03b8s mult",  "vals": ths_vals,  "fmt": "{:.3f}x"},
...
def plot_hydrograph_envelope(hydros_subset, df_subset, title_suffix, filename,
    ...
    ax.set_title(
        ...
        f"11-parameter LHS  |  {SERIES_TITLE}  |  {EVENT_LABEL}",
        fontsize=12
    )
...
# -----------------------------------------------------------------------
# FIGURE 3: Pearson correlation bar chart — 7 bars
# -----------------------------------------------------------------------
...
ax.set_title(
    f"Parameter–KGE correlations — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
...
# -----------------------------------------------------------------------
# FIGURE 4: Parallel coordinates — 7 axes
# -----------------------------------------------------------------------
...
ax.set_title(
    f"Parallel coordinates — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
    f"Lines colored by KGE  |  {EVENT_LABEL}  |  f, As, Au fixed",
    fontsize=12
)
...
# -----------------------------------------------------------------------
# FIGURE 5: Pairwise scatter matrix — C(7,2) = 21 panels
# -----------------------------------------------------------------------
print("Figure 5: Pairwise scatter matrix (21 panels)")

pairs  = list(combinations(range(n_params), 2))   # 21 pairs for 7 params
n_cols = 4
n_rows = (len(pairs) + n_cols - 1) // n_cols       # 6 rows
...
sm = plt.cm.ScalarMappable(cmap=KGE_CMAP, norm=KGE_NORM)
sm.set_array([])
fig.colorbar(sm, ax=axes.ravel().tolist(), label="KGE", shrink=0.5, pad=0.02)
fig.suptitle(
    f"Pairwise parameter scatter — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
...
# -----------------------------------------------------------------------
# FIGURE 6: KGE vs each parameter — 7 panels (4 top + 3 bottom)
# -----------------------------------------------------------------------
print("Figure 6: KGE vs each parameter (1D response)")

fig = plt.figure(figsize=(16, 10))
gs  = fig.add_gridspec(2, 8, hspace=0.45, wspace=0.45)
...
fig.suptitle(
    f"KGE response to each parameter — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
    f"{EVENT_LABEL}  |  Dashed = rolling median  |  f, As, Au fixed",
    fontsize=12
)
...
ax.set_title(
    f"PBIAS vs KGE — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
...
ax.set_title(
    f"Top 15 runs by KGE — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
    f"{EVENT_LABEL}  |  f=0.020 mm^-1, As=Au=1 fixed  |  Green row = best run",
    fontsize=11, pad=12
)
fig.tight_layout()
...
fig.suptitle(
    f"KGE component decomposition vs Ks — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
...
# -----------------------------------------------------------------------
# FIGURE 10: New parameters deep-dive — channelwidthcoeff & thetaS_mult
#   (a) KGE vs channelwidthcoeff, colored by Ks
#   (b) KGE vs thetaS_mult, colored by Ks
#   (c) channelwidthcoeff × thetaS_mult, colored by KGE (interaction map)
# -----------------------------------------------------------------------
print("Figure 10: New parameters deep-dive (channelwidthcoeff & thetaS_mult)")

window = max(5, len(df) // 8)

fig, axes = plt.subplots(1, 3, figsize=(16, 5.5))

# --- Panel (a): KGE vs channelwidthcoeff, colored by Ks ---
ax = axes[0]
sc_a = ax.scatter(cw_vals, kge_vals, c=ks_vals, cmap="viridis",
                  s=35, edgecolors="white", linewidths=0.4, alpha=0.85, zorder=3)
ax.scatter(cw_vals[best_idx], kge_vals[best_idx],
           s=180, marker="*", color="white", edgecolors="black",
           linewidths=1.2, zorder=5, label=f"Best KGE={best_kge:.3f}")
sort_order = np.argsort(cw_vals)
ys_sm = pd.Series(kge_vals[sort_order]).rolling(window, center=True, min_periods=1).median().values
ax.plot(cw_vals[sort_order], ys_sm, color="navy", linewidth=1.6, linestyle="--",
        alpha=0.7, label="Rolling median")
ax.axhline(0, color="gray", linewidth=0.8, linestyle=":", alpha=0.6)
ax.set_xlabel("Channel width coeff. \u03b1B", fontsize=10)
ax.set_ylabel("KGE", fontsize=10)
ax.set_title("KGE vs \u03b1B\n(colored by Ks mult)", fontsize=11)
...
# --- Panel (b): KGE vs thetaS_mult, colored by Ks ---
ax = axes[1]
sc_b = ax.scatter(ths_vals, kge_vals, c=ks_vals, cmap="viridis",
                  s=35, edgecolors="white", linewidths=0.4, alpha=0.85, zorder=3)
ax.scatter(ths_vals[best_idx], kge_vals[best_idx],
           s=180, marker="*", color="white", edgecolors="black",
           linewidths=1.2, zorder=5, label=f"Best KGE={best_kge:.3f}")
sort_order = np.argsort(ths_vals)
ys_sm = pd.Series(kge_vals[sort_order]).rolling(window, center=True, min_periods=1).median().values
ax.plot(ths_vals[sort_order], ys_sm, color="navy", linewidth=1.6, linestyle="--",
        alpha=0.7, label="Rolling median")
ax.axhline(0, color="gray", linewidth=0.8, linestyle=":", alpha=0.6)
ax.set_xlabel("Saturated moisture \u03b8s multiplier", fontsize=10)
ax.set_ylabel("KGE", fontsize=10)
ax.set_title("KGE vs \u03b8s mult\n(colored by Ks mult)", fontsize=11)
fig.colorbar(sc_b, ax=ax, label="Ks multiplier", shrink=0.85)
ax.legend(fontsize=8, loc="lower right", facecolor="white", framealpha=0.85)
ax.grid(alpha=0.25)

# --- Panel (c): channelwidthcoeff × thetaS_mult interaction, colored by KGE ---
ax = axes[2]
sc_c = ax.scatter(cw_vals, ths_vals, c=kge_vals, cmap=KGE_CMAP, norm=KGE_NORM,
                  s=35, edgecolors="white", linewidths=0.4, alpha=0.85, zorder=3)
ax.scatter(cw_vals[best_idx], ths_vals[best_idx],
           s=180, marker="*", color="white", edgecolors="black",
           linewidths=1.2, zorder=5, label=f"Best KGE={best_kge:.3f}")
ax.set_xlabel("Channel width coeff. \u03b1B", fontsize=10)
ax.set_ylabel("Saturated moisture \u03b8s multiplier", fontsize=10)
ax.set_title("\u03b1B \u00d7 \u03b8s interaction\n(colored by KGE)", fontsize=11)
fig.colorbar(sc_c, ax=ax, label="KGE", shrink=0.85)
ax.legend(fontsize=8, loc="upper right", facecolor="white", framealpha=0.85)
ax.grid(alpha=0.25)

fig.suptitle(
    f"New-parameter deep-dive — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
    f"{EVENT_LABEL}  |  \u03b1B baseline=2.33 (Ivanov 2004)  |  \u03b8s constrained > \u03b8*s=0.37",
    fontsize=11
)
fig.tight_layout()
save_fig(fig, "fig10_new_params_deepdive.png")


# -----------------------------------------------------------------------
# FIGURE 11: psiB_mult deep-dive — carried from series 82
#   (a) KGE vs psiB_mult, colored by Ks
#   (b) psiB_mult × Ks_mult, colored by KGE
#   (c) PBIAS vs psiB_mult, colored by KGE
# -----------------------------------------------------------------------
print("Figure 11: psiB_mult deep-dive")
...
# --- Panel (b): psiB_mult × Ks_mult, colored by KGE ---
ax = axes[1]
...
ax.plot(psib_vals[sort_order], ys_sm, color="navy", linewidth=1.6, linestyle="--",
        alpha=0.7, label="Rolling median")
ax.set_xlabel("psiB multiplier", fontsize=10)
...
fig.suptitle(
    f"psiB_mult sensitivity deep-dive — 11-param LHS  {SERIES_TITLE}  (n={len(df)})\n"
    f"{EVENT_LABEL}  |  Baseline PsiB per class: RS=\u2212390, CO=\u2212401, CeD=\u2212183, EbD=\u2212450, Cb=\u2212117 mm",
    fontsize=11
)
fig.tight_layout()
save_fig(fig, "fig11_psib_sensitivity.png")
