...
plot_lhs_Ks_f_97.py
====================
Generates contour plots from the Series 97 synthetic Ks_mult x f_RS_abs LHS
sweep (lhs_results_synth_Ks_f_97.csv), scored against the noise-free
synthetic truth hydrograph (cv/r/n pinned at confirmed truth values:
cv=4.5, r=0.24, n=0.026).

This is a Series-97-specific companion to plot_lhs_Ks_pair.py, not a
replacement for it -- the generic Ks x <param> script is for real-gauge
sweeps (Series 70-style) and stays as-is. This script adds three things
that are specific to reading the Series 97 surface and aren't needed for
the general case, so they live here rather than getting bolted onto the
consolidated script:

  1. Every single-metric contour is drawn twice, side-by-side: linear f
     (left) and log10 f (right). f_RS_abs spans 0.003-0.05 (>1 order of
     magnitude), and the low-f half of that range -- where the Series 96
     anchors and the PBIAS-zero valley live -- gets compressed into a thin
     band under linear scaling. Log scale spreads it out.
  2. KGE, NSE, and PBIAS colorbars are clipped (vmin/vmax + extend='both'
     or 'min') so the very poor Ks>=9 band doesn't eat most of the color
     range and wash out the mid-Ks (4-8x) structure, which is where the
     valley/swoosh shape actually needs to be readable.
  3. A top-20-by-|PBIAS| table is written to CSV and printed to console.

Usage (run from the smf_demo directory):
    python plot_lhs_Ks_f_97.py

Produces 7 figures + 1 CSV saved to:
    calibration_work/03_comparisons/sensitivity_plots/lhs_Ks_f_97/
...
Figure list:
    ...
    top20_pbias_synth_97.csv      -- 20 runs with smallest |PBIAS|
"""
...
# CONFIG
# ======================================================================
RESULTS_CSV   = "lhs_results_synth_Ks_f_97log.csv"
OUTPUT_SUBDIR = "lhs_Ks_f_97log"
SECOND_COL    = "f_RS_abs"
SECOND_LABEL  = "Hydraulic conductivity decay f (RS soil, mm\u207b\u00b9)"
EVENT_LABEL   = "SMF Aug 12, 2014 (synthetic truth, Series 97log)"
PAIR_LABEL    = "Ks x f"

# Clipped colorbar ranges -- chosen to give the mid-Ks (4-8x) valley
# structure full color resolution instead of stretching to cover the
# Ks>=9 band, where KGE bottoms out near -0.4 and PBIAS reaches -72%/+93%.
...
if not results_path.exists():
    raise FileNotFoundError(
        ...
        "Run run_lhs_synth_Ks_f_97.py first."
    )
...
top20_path = plot_dir / "top20_pbias_synth_97log.csv"
...
for log_scale, suffix, axis_label in [(False, "linear", "linear f axis"),
    ...
    fig.suptitle(
        ...
        f"KGE/NSE/PBIAS colorbars clipped to reduce Ks\u22659 washout",
        fontsize=13
    )
...
#
# Purpose: the contourf/griddata panels above show dense vertical banding
# in KGE/NSE/PBIAS/alpha/beta, and a heavily zigzagging PBIAS=0 contour
# line bleeding into every panel it's overlaid on (including r). This
# figure sidesteps griddata entirely -- it bins runs into a few narrow
# f-RS_abs slices and plots raw (kge, pbias) vs Ks_mult directly, so any
# fine-scale "banding" that shows up here is real point-to-point run
# noise, and anything that DOESN'T show up here (i.e. only appears in
# the interpolated panels) is a cubic-interpolation/contour artifact.
# -----------------------------------------------------------------------
N_F_BINS = 4
...
fig, axes = plt.subplots(2, N_F_BINS, figsize=(4.5 * N_F_BINS, 8), sharex=True)
...
for j, f_center in enumerate(f_bin_centers):
    ...
    ax_pbias.set_ylabel("PBIAS % (raw)" if j == 0 else "")
...
fig.suptitle(
    ...
    f"point order only (not a fit)  |  Dotted gray = best sampled KGE, dashed black = PBIAS=0",
    fontsize=12
)
fig.tight_layout()
