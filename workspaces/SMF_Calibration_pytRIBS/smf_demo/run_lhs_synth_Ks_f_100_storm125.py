"""
run_lhs_synth_Ks_f_100_storm125.py
====================================
Series 100_storm125 -- sibling of Series 100, scored against a SCALED-
RAINFALL truth instead of the baseline forcing. Rain intensity (R column
only, both SMF and SMPHQ gauges) is scaled to 125% of the original event
...
Differences from run_lhs_synth_Ks_f_100.py:
  - met.gaugestations points at Master_Precip_storm125.sdf (via GAUGE_SDF
...
REQUIRES exactly one *.qout file in calibration_work/synth_truth/storm125/
before running. The script checks this itself at startup and will refuse
to run otherwise.

Usage (run from the smf_demo directory):
    python run_lhs_synth_Ks_f_100_storm125.py                # 200 samples, seed=42 (full sweep)
    python run_lhs_synth_Ks_f_100_storm125.py --n 35 --seed 7  # reduced pilot -- NOTE: a
                                                                 ...
                                                                 # storm080 sibling -- use it
                                                                 # only to smoke-test the
                                                                 # forcing/truth wiring and
                                                                 # check for hang-risk-zone
                                                                 # behavior, not for swoosh
                                                                 # analysis.
    python run_lhs_synth_Ks_f_100_storm125.py --skip_existing  # resume interrupted run
    python run_lhs_synth_Ks_f_100_storm125.py --timeout 600    # more generous per-run timeout

Output:
    calibration_work/03_comparisons/summary_tables/lhs_results_synth_Ks_f_100_storm125.csv
    calibration_work/03_comparisons/summary_tables/lhs_results_synth_Ks_f_FAILED_100_storm125.csv
"""
...
# ------------------------------------------------------------------
STORM_LABEL = "storm125"
STORM_SCALE = 1.25   # informational only (already baked into GAUGE_SDF's forcing files)
...
def build_only(ks_mult, f_rs_abs):
    ...
    try:
        ...
        met.gaugestations['value']    = GAUGE_SDF   # storm125-scaled forcing, not baseline
...
def main():
    parser = argparse.ArgumentParser(
        description="Series 100_storm125 -- joint Ks_mult x f_RS_abs LHS "
                    "sweep against the synthetic truth (Ks_mult=7.0x, "
                    "f_RS_abs=0.012) under 125%-scaled rainfall forcing, "
    ...
    # ------------------------------------------------------------------
    # SAFETY CHECK: confirm the storm125 truth subdirectory has exactly
    ...
    if len(qout_files) != 1:
        raise RuntimeError(
            ...
            f"storm125 synthetic-truth mode, found {len(qout_files)}: "
            f"{[f.name for f in qout_files]}. This sweep scores against "
            f"the storm125-scaled-forcing truth specifically -- confirm "
            f"the storm125 truth .qout lives in this subdirectory (see "
            f"scale_precip_forcing.py / truth generation notes) before "
            f"running, or results won't be scored against the truth you "
            f"expect."
        )
    print(f"Storm125 synthetic truth mode confirmed active: {qout_files[0].name}")
    ...
    print(f"LHS sweep -- Series {LHS_SERIES} -- Ks_mult x f_RS_abs vs SYNTHETIC TRUTH "
          f"(storm125 forcing, {STORM_SCALE:.0%} rainfall)")
