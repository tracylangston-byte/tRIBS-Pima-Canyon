"""
run_sensitivity_single_interp.py
=================================
INTERPOLATION-RESAMPLE VARIANT of run_sensitivity_single.py.

Identical to run_sensitivity_single.py in every respect EXCEPT one line:
the simulated streamflow is resampled to the 5-minute comparison grid via
time-interpolation, not mean-aggregation.

Why this variant exists
------------------------
In run_sensitivity_single.py, the observed series is already resampled
with `.resample('5min').interpolate(method='time')` when scoring against
synthetic truth (obs is the .qout truth file itself, native ~3.75-min
timestep). But the SIMULATED series is always resampled with
`.resample('5min').mean()`, regardless of mode -- including in synthetic
truth mode, where the sim output is written directly on a 5-minute grid
(opintrvl=0.0833 hr in build_only()). If the sim's native timestamps
aren't exactly aligned to clean 5-minute boundaries, bin-mean averaging
can blend two straddling points together, which mechanically shaves the
top off a fast-changing peak -- exactly where Handoff_Series100_
TruthPointAnomaly_v4.md section 7.7 localized the entire truth-point
volume residual (a ~45-minute window at and immediately after the peak;
15+ hours of recession/tail track almost perfectly). Time-interpolation
reads the value at the exact 5-minute mark instead of averaging a bin,
so it should not have this smoothing effect.

This script does NOT change obs handling at all (still mode-dependent,
matching the original). It only swaps the sim resample line:

    OLD: strmflw_sim['Qstrm_m3s'].resample('5min').mean()
    NEW: strmflw_sim['Qstrm_m3s'].resample('5min').interpolate(method='time')

Everything else -- metric formulas, phase-metric logic, output schema,
column names, file-naming convention -- is byte-for-byte the same as
run_sensitivity_single.py, so downstream CSVs from this script are
directly comparable to the mean-aggregation ones.

Usage: same calling convention as run_sensitivity_single.py -- reads
calibration_work/current_run_config.json, run from the smf_demo directory.
Not meant to be swapped in for every series; used specifically by
validate_truth_point_100_storms_v2.py for the resampling-method A/B test.
"""
...
# -----------------------------------------------------------------------
# HELPER: phase-specific metrics (unchanged from run_sensitivity_single.py)
# -----------------------------------------------------------------------
def _compute_phase_metrics(obs: pd.Series, sim: pd.Series) -> dict:
    ...
    #    Count timesteps above threshold x dt_min.
...
# -----------------------------------------------------------------------
# HELPER: detect synthetic truth mode (unchanged from run_sensitivity_single.py)
# -----------------------------------------------------------------------
def _find_synth_truth_file(calib_dir: Path, truth_file_override=None):
    ...
    See run_sensitivity_single.py for the full docstring -- logic here is
    identical, only reproduced so this module has no import dependency on
    the sibling script.
    """
...
def run_and_score():
    ...
    # (unchanged from run_sensitivity_single.py)
    # ------------------------------------------------------------------
    ...
    #
    # *** THE ONLY SUBSTANTIVE CHANGE FROM run_sensitivity_single.py ***
    # obs handling is untouched (still mode-dependent, matching the
    # original exactly). sim now uses time-interpolation instead of
    # mean-aggregation, for the reasons in the module docstring above.
    # ------------------------------------------------------------------
    ...
    # CHANGED: was .resample('5min').mean() -- see module docstring.
    sim_resampled = (strmflw_sim['Qstrm_m3s']
                     .resample('5min')
                     .interpolate(method='time'))
    ...
    # ------------------------------------------------------------------
    # Compute standard metrics (unchanged formulas)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Assemble metrics summary (identical schema to run_sensitivity_single.py)
    # ------------------------------------------------------------------
    metrics_summary = {
        # --- identifiers ---
        "run_id":              run_id,
        ...
        "resample_method":     "interpolate",   # NEW: tags this variant's output
