...
==========================
Latin Hypercube Sampling sweep for the Series 93 synthetic inversion
experiment to try to create a hydrograph with a different shape. 
Identical to Series 92 except for change in channelroughness truth value.
...
Parameters swept (4 free dimensions):
    ...
    channelroughness: 0.02 – 0.10  (true = 0.075, 69th pct of range —
                                    0.075 is a test case for heavy brush
 
Fixed (not swept):
    f_RS_abs: 0.020 mm^-1
 
Truth exclusion:
    Any LHS sample matching all four truth values within TRUTH_TOL=1e-6
    is skipped. The truth run (SMF_20140812_60_Ks8p5x) must not appear
    in the inversion ensemble.
 
KGE ceiling:
    ~0.912 (resampling asymmetry between sim .mean() and obs
    .interpolate()). Top runs near the ceiling with parameters near
    truth = successful recovery.
 
Series: 93
Output: calibration_work/03_comparisons/summary_tables/lhs_results_synth_4param_92.csv
 
Usage (run from the smf_demo directory):
    python run_lhs_synth_4param_92.py              # 50 samples, seed=42
    python run_lhs_synth_4param_92.py --n 100      # recommended
    python run_lhs_synth_4param_92.py --seed 99    # different seed
    python run_lhs_synth_4param_92.py --skip_existing  # resume interrupted run
"""
...
# True value must be interior to [lo, hi].
# ------------------------------------------------------------------
LHS_PARAMS = {
    ...
    "channelroughness": {"lo": 0.02, "hi": 0.10},  # true = 0.075 (69th pct)
    #                                                 ^ changed from 0.026 in S92
}
...
# Series metadata
LHS_SERIES   = "93"
LHS_CATEGORY = "93_lhs_synth_inversion"
...
def main():
    parser = argparse.ArgumentParser(
        description="Series 93 synthetic inversion LHS sweep — 4 free parameters.")
    ...
    out_path = summary_dir / "lhs_results_synth_4param_93.csv"
